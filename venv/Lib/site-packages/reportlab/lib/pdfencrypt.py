...
class StandardEncryption:
    ...
    def __init__(self, userPassword, ownerPassword=None, canPrint=1, canModify=1, canCopy=1, canAnnotate=1,
                 strength=None):
        '''
    ...
    def info(self):
        ...
        if not self.prepared:
            raise ValueError("encryption not prepared!")
        return StandardEncryptionDictionary(O=self.O, OE=self.OE, U=self.U, UE=self.UE, P=self.P, Perms=self.Perms,
...
def encryptionkey(password, OwnerKey, Permissions, FileId1, revision=None):
    ...
    if revision == 2:
        ...
        key = md5output[:16]
    if DEBUG: print('encryptionkey(%s,%s,%s,%s,%s)==>%s' % tuple(
...
def computeO(userPassword, ownerPassword, revision):
    ...
    ascii(PadString), ascii(ownerPad), ascii(password), ascii(userPad), ascii(digest), revision))
    if revision == 2:
    ...
    if DEBUG: print(
        'computeO(%s,%s,%s)==>%s' % tuple([hexText(str(x)) for x in (userPassword, ownerPassword, revision, O)]))
    return O
...
def computeU(encryptionkey, encodestring=PadString, revision=None, documentId=None):
    ...
    if revision == 2:
        ...
        while len(tmp) < 32:
            tmp += b'\0'
        result = tmp
    if DEBUG: print('computeU(%s,%s,%s,%s)==>%s' % tuple(
        [hexText(str(x)) for x in (encryptionkey, encodestring, revision, documentId, result)]))
    return result
...
def encodePDF(key, objectNumber, generationNumber, string, revision=None):
    ...
    if DEBUG: print('encodePDF(%s,%s,%s,%s,%s)==>%s' % tuple(
        [hexText(str(x)) for x in (key, objectNumber, generationNumber, string, revision, encrypted)]))
    return encrypted
