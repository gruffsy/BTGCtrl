...
if isPy3:
    ...
    def isNonPrimitiveInstance(x):
        return isinstance(x, types.InstanceType) or not isinstance(x, (
        float, int, long, type, tuple, list, dict, bool, unicode, str, buffer, complex, slice, types.NoneType,
...
try:
    ...
    def rl_glob(pattern, glob=glob.glob, fnmatch=fnmatch.fnmatch, _RL_DIR=_RL_DIR, pjoin=os.path.join):
        ...
        r = glob(pfn)
        if c or r == []:
            r += list(map(lambda x, D=_archivepfx, pjoin=pjoin: pjoin(_archivepfx, x), list(
                filter(lambda x, pfn=pfn, fnmatch=fnmatch: fnmatch(x, pfn), list(__rl_loader__._files.keys())))))
        return r
except:
...
def rl_getmtime(pn, os_path_isfile=os.path.isfile, os_path_normpath=os.path.normpath, os_path_getmtime=os.path.getmtime,
    ...
    s = e[5]
    d = e[6]
    return time_mktime((((d >> 9) & 0x7f) + 1980, (d >> 5) & 0xf, d & 0x1f, (s >> 11) & 0x1f, (s >> 5) & 0x3f,
                        (s & 0x1f) << 1, 0, 0, 0))
...
class ImageReader(object):
    ...
    def identity(self):
        ...
        ident = self._ident
        return '[%s@%s%s%s]' % (self.__class__.__name__, hex(id(self)), ident and (' ident=%r' % ident) or '',
                ...
                if sys.platform[0:4] == 'java':
                    ...
                    if mode in ('LA', 'RGBA'):
                        ...
                        if im.format == 'PNG' and im.mode == 'P' and 'transparency' in im.info:
                            ...
                            im = im.convert('RGB')
                        self.mode = 'RGB'
                    self._data = (im.tobytes if hasattr(im,
