...
from reportlab.lib.utils import import_zlib, open_for_read, makeFileName, isSeq, isBytes, isUnicode, _digester, isStr, \
    bytestr, isPy3, annotateException, TimeStamp
...
class PDFPage(PDFCatalog):
    ...
    def check_format(self, document):
        ...
        self.MediaBox = self.MediaBox or PDFArray(
            self.Rotate in (90, 270) and [0, 0, self.pageheight, self.pagewidth] or [0, 0, self.pagewidth,
                                                                                     self.pageheight])
        if not self.Annots:
            self.Annots = None
        else:
...
class Destination(PDFObject):
    ...
    def format(self, document):
        ...
        if f is None: raise ValueError(
            "format not resolved, probably missing URL scheme or undefined destination target for '%s'" % self.name)
        p = self.page
        if p is None: raise ValueError(
            "Page not bound, probably missing URL scheme or undefined destination target for '%s'" % self.name)
        f.page = p
