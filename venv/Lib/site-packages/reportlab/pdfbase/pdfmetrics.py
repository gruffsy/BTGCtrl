...
from reportlab.lib.utils import rl_isfile, rl_glob, rl_isdir, open_and_read, open_and_readlines, findInPaths, isSeq, \
    isStr, isUnicode, isPy3
...
if isPy3:
    def _pfbCheck(p, d, m, fn):
        if chr(d[p]) != PFB_MARKER or chr(d[p + 1]) != m:
            raise ValueError('Bad pfb file\'%s\' expected chr(%d)chr(%d) at char %d, got chr(%d)chr(%d)' % (
