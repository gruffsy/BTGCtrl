...
uri_pat = re.compile(
    u'(^(?:[a-z0-9\\.\\-\\+]*)://)(?:\\S+(?::\\S*)?@)?(?:(?:25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)(?:\\.(?:25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)){3}|\\[[0-9a-f:\\.]+\\]|([a-z\xa1-\uffff0-9](?:[a-z\xa1-\uffff0-9-]{0,61}[a-z\xa1-\uffff0-9])?(?:\\.(?!-)[a-z\xa1-\uffff0-9-]{1,63}(?<!-))*\\.(?!-)(?:[a-z\xa1-\uffff-]{2,63}|xn--[a-z0-9]{1,59})(?<!-)\\.?|localhost))(?::\\d{2,5})?(?:[/?#][^\\s]*)?\\Z',
    re.I)
...
def _hyphenateFragWord(hyphenator, FW, newWidth, maxWidth, uriWasteReduce, embeddedHyphenation,
    ...
    if len(FW) == 2:
        ...
        R = _hyGenPair(hyphenator, s, ww, newWidth, maxWidth, f.fontName, f.fontSize, uriWasteReduce,
                       embeddedHyphenation, hymwl)
        if R:
            jc, hylen, hw, tw, h, t = R
            return [(_SplitFragHY if jc else _SplitFragH)([hw + hylen, (f, h + jc)]),
        ...
        if R:
            jc, hylen, hw, tw, h, t = R
            return [(_SplitFragHY if jc else _SplitFragH)(h),
...
def makeCJKParaLine(U, maxWidth, widthUsed, extraSpace, lineBreak, calcBounds):
    ...
    return FragLine(kind=1, extraSpace=extraSpace, wordCount=1, words=words[1:], fontSize=maxSize, ascent=maxAscent,
                    descent=minDescent, maxWidth=maxWidth, currentWidth=widthUsed, lineBreak=lineBreak)
    ...
    def breakLines(self, width):
        ...
        if hyphenator:
            if isStr(hyphenator):
                ...
                raise ValueError(
                    'hyphenator should be a language spec or a callable unicode -->  pairs not %r' % hyphenator)
        else:
            hyphenator = None
        ...
        if (nFrags == 1
                ...
                if newWidth > maxWidth + spaceShrink and not isinstance(word, _SplitWordH):
                    ...
                    if splitLongWords and not isinstance(word, _SplitWord):
                        ...
                        if wordWidth > max(maxWidths[nmw:nmw + 1]):
                            # a long word
                            words[0:0] = _splitWord(word, maxWidth - spaceWidth - currentWidth, maxWidths, lineno,
            ...
            while _words:
                ...
                if not endLine:
                    ...
                    spaceWidth = stringWidth(' ', fontName, fontSize) if isinstance(w,
                                                                                    _HSFrag) else 0  # of the space following this word
                    ...
                    for i in w[2:]:
                        ...
                        if calcBounds:
                            if g._fkind == _FK_IMG:
                                descent, ascent = imgVRange(imgNormV(g.cbDefn.height, fontSize), g.cbDefn.valign,
                                                            fontSize)
                            else:
                    ...
                    lines.append(FragLine(extraSpace=maxWidth - currentWidth, wordCount=n,
                                          lineBreak=lineBreak and njlbv, words=words, fontSize=maxSize,
                                          ascent=maxAscent, descent=minDescent, maxWidth=maxWidth,
                                          sFW=sFW))
                    sFW = len(FW) - 1
    ...
    def dumpFrags(frags, indent=4, full=False):
        ...
        for i, f in enumerate(frags):
            if full:
                ...
                aR('[%r, %s], #%d %s' % (
                f[0], ', '.join(('(%s,%r)' % (fx[0].__class__.__name__, fx[1]) for fx in f[1:])), i,
