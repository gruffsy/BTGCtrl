    ...
    def wrap(self, availWidth, availHeight):
        ...
        for (level, text, pageNum, key) in _tempEntries:
            ...
            if key:
                ...
                keyVal = None
            para = Paragraph('%s<onDraw name="drawTOCEntryEnd" label="%d,%d,%s"/>' % (text, pageNum, level, keyVal),
                             style)
...
class SimpleIndex(IndexingFlowable):
    ...
    def _build(self, availWidth, availHeight):
        ...
        def getkey(seq):
            return [''.join((c for c in unicodedata.normalize('NFD', x.upper()) if unicodedata.category(c) != 'Mn')) for
                    x in seq[0]]
        ...
        for texts, pageNumbers in _tempEntries:
            ...
            nalpha = ''.join(
                (c for c in unicodedata.normalize('NFD', texts[0][0].upper()) if unicodedata.category(c) != 'Mn'))
            if alpha != nalpha:
                alpha = nalpha
                if self.headers:
                    header = alpha
