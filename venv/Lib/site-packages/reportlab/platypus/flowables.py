...
class KeepTogether(_ContainerSpace, Flowable):
    ...
    def identity(self, maxLen=None):
        ...
        self.__class__.__name__, hex(id(self)), self._frameName(), "\n".join([f.identity() for f in self._content]))
        if maxLen:
            return msg[0:maxLen]
        else:
            return msg
...
class KeepInFrame(_Container, Flowable):
    def __init__(self, maxWidth, maxHeight, content=[], mergeSpace=1, mode='shrink', name='', hAlign='LEFT',
                 vAlign='BOTTOM', fakeWidth=None):
    ...
    def identity(self, maxLen=None):
        return "<%s at %s%s%s> size=%sx%s" % (self.__class__.__name__, hex(id(self)), self._frameName(),
                                              ...
                                              getattr(self, 'maxWidth', '') and (
                                                          ' maxWidth=%s' % fp_str(getattr(self, 'maxWidth', 0))) or '',
                                              getattr(self, 'maxHeight', '') and (
                                                          ' maxHeight=%s' % fp_str(getattr(self, 'maxHeight'))) or '')
...
class ImageAndFlowables(_Container, _FindSplitterMixin, Flowable):
    ...
    def drawOn(self, canv, x, y, _sW=0):
        ...
        if self.imageHref:
            canv.linkURL(self.imageHref, (Ix, y + self.height - self._itpad - self._hI, Ix + self._wI, y + self.height),
                         relative=1)
    ...
    def drawOn(self, canv, x, y, _sW=0):
        if self._width > _FUZZ and self._height > _FUZZ:
            ...
            if st or self._fillColor:
                ...
                canv.rect(self._x, self._y, self._width, self._height, stroke=1 if st else 0,
        ...
        if doBox or doVLines:
            ...
            if doVLines:
                ...
                for i in xrange(1, nCols):
                    ...
                    vLines.append(_AbsLine(vlx, oby2, vlx, oby1, strokeWidth=self._vLinesStrokeWidth,
...
class FrameBG(AnchorFlowable):
    ...
    def __repr__(self):
        return "%s(%s)" % (self.__class__.__name__,
                           ', '.join(['%s=%r' % (i, getattr(self, i, None)) for i in 'start color left right'.split()]))
        ...
        for d, f in self._flowablesIter():
            ...
            if d:
                ...
                if b:
                    if b.bulletType != bt:
                        raise ValueError(
                            'Included LIIndenter bulletType=%s != OrderedList bulletType=%s' % (b.bulletType, bt))
                    value = int(b.value)
                else:
