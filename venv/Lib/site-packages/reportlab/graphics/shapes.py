...
class Drawing(Group, Flowable):
    ...
    _xtraAttrMap = AttrMap(
        ...
        hAlign=AttrMapValue(OneOf("LEFT", "RIGHT", "CENTER", "CENTRE"),
    ...
    def draw(self, showBoundary=_unset_):
        ...
        renderPDF.draw(self, self.canv, 0, 0,
                       showBoundary=showBoundary if showBoundary is not _unset_ else getattr(self, '_showBoundary',
                                                                                             _unset_))
    ...
    def save(self, formats=None, verbose=None, fnRoot=None, outDir=None, title='', **kw):
        ...
        if not os.path.isabs(outDir): outDir = os.path.join(
        ...
        if 'pdf' in plotMode:
            ...
            renderPDF.drawToFile(self, filename, title,
            ...
            if bmFmt in plotMode:
                ...
                if dtc:
                    ...
                    if bmFmt in dtcfmts and not getattr(dtc, 'disabled', 0):
                        ...
                        dtc = None
                renderPM.drawToFile(self, filename, fmt=bmFmt,
                                    showBoundary=getattr(self, 'showBorder', rl_config.showBoundary),
    ...
    def asString(self, format, verbose=None, preview=0, **kw):
        ...
        assert format in (
        'pdf', 'ps', 'eps', 'gif', 'png', 'jpg', 'jpeg', 'bmp', 'ppm', 'tiff', 'tif', 'py', 'pict', 'pct', 'tiffp',
        'tiffl', 'tiff1'), 'Unknown file format "%s"' % format
        from reportlab import rl_config
        # verbose = verbose is not None and (verbose,) or (getattr(self,'verbose',verbose),)[0]
        if format == 'pdf':
            from reportlab.graphics import renderPDF
...
class LineShape(Shape):
    ...
    _attrMap = AttrMap(
        ...
        strokeOpacity=AttrMapValue(isOpacity,
...
class SolidShape(LineShape):
    ...
    _attrMap = AttrMap(BASE=LineShape,
                       ...
                       fillOpacity=AttrMapValue(isOpacity,
...
class ArcPath(Path):
    ...
    def addArc(self, centerx, centery, radius, startangledegrees, endangledegrees, yradius=None, degreedelta=None,
               moveTo=None, reverse=None):
        P = getArcPoints(centerx, centery, radius, startangledegrees, endangledegrees, yradius=yradius,
                         degreedelta=degreedelta, reverse=reverse)
