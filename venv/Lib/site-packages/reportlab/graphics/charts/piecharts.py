...
def boundsOverlap(P, Q):
    return not (P[0] > Q[2] - 1e-2 or Q[0] > P[2] - 1e-2 or P[1] > (0.5 * (Q[1] + Q[3])) - 1e-2 or Q[1] > (
                0.5 * (P[1] + P[3])) - 1e-2)
...
class Pie(AbstractPieChart):
    _attrMap = AttrMap(BASE=AbstractPieChart,
                       ...
                       simpleLabels=AttrMapValue(isBoolean,
                       ...
                       other_threshold=AttrMapValue(isNumber, desc='A value for doing threshholding, not used yet.',
                                                    advancedUsage=1),
                       checkLabelOverlap=AttrMapValue(EitherOr((isNumberInRange(0.05, 1), isBoolean)),
    ...
    def makePointerLabels(self, angles, plMode):
        ...
        for i, A in angles:
            ...
            if ri: nr += 1
            if li: nl += 1
            l._origdata = dict(bounds=b, width=w, height=h, li=li, ri=ri, index=i, edgePad=style.label_pointer_edgePad,
                               piePad=style.label_pointer_piePad, elbowLength=style.label_pointer_elbowLength)
            maxW = max(w, maxW)
            sumH += h + 2
...
class LegendedPie(Pie):
    ...
    _attrMap = AttrMap(BASE=Pie,
                       ...
                       legendNumberFormat=AttrMapValue(None,
                                                       desc="Formatting routine for number on right hand side of legend."),
                       legendNumberOffset=AttrMapValue(isNumber,
                       ...
                       legend_data=AttrMapValue(isNoneOrListOfNoneOrNumbers,
        ...
        for f in range(len(self.data)):
            if self.legend_names == None:
                ...
                try:
                    ...
                    self.legend1.colorNamePairs.append(
                        (self.pieAndLegend_colors[f % len(self.pieAndLegend_colors)], self.legend_names[f]))
            if self.legend_data != None:
                ...
                if ldf is None or lNF is None:
                    ...
                    raise ValueError(
                        "Unknown formatter type %s, expected string or function" % ascii(self.legendNumberFormat))
