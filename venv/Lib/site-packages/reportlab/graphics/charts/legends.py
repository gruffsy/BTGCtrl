...
class SubColProperty(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        underlines=AttrMapValue(
            EitherOr((NoneOr(isInstanceOf(Line)), SequenceOf(isInstanceOf(Line), emptyOK=0, lo=0, hi=0x7fffffff))),
...
class Legend(Widget):
    ...
    _attrMap = AttrMap(
        ...
        dividerLines=AttrMapValue(OneOf(0, 1, 2, 3, 4, 5, 6, 7),
        ...
        for i in xrange(n):
            if autoCP:
                ...
                if isAuto(swatchMarker):
                    ...
                    col.index = i
                if isAuto(name):
                    name = getattr(swatchMarker, 'chart', getattr(swatchMarker, 'obj', None)).getSeriesName(i,
                                                                                                            'series %d' % i)
            T = _getLines(name)
            S = []
            aS = S.append
            ...
            if isAuto(col):
                ...
                if isSymbol(swatchMarker):
                    ...
                    c = self._defaultSwatch(swatchX, swatchY, dx, dy, fillColor=col, strokeWidth=strokeWidth,
                                            strokeColor=strokeColor)
