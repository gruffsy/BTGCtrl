...
class LineChartProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        lineStyle=AttrMapValue(NoneOr(OneOf('line', 'joinedLine', 'bar')), desc="What kind of plot this line is",
                               advancedUsage=1),
        barWidth=AttrMapValue(isNumberOrNone, desc="Percentage of available width to be used for a bar",
                              advancedUsage=1),
...
class AbstractLineChart(PlotArea):
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        if lineStyle == 'bar':
            ...
            L = Rect(x, y, width, height, strokeWidth=strokeWidth, strokeColor=color, strokeLineCap=0,
                     strokeDashArray=dash, fillColor=getattr(style, 'fillColor', color))
        ...
        for rowNo, row in enumerate(reversed(P) if self.reversePlotOrder else P):
            ...
            if lineStyle == 'bar':
                ...
                if isinstance(barWidth, Percentage):
                    ...
                    hbw = barWidth * 0.5
                for x, y in row:
                    g.add(Rect(x - hbw, min(y, yzero), 2 * hbw, abs(y - yzero), strokeWidth=strokeWidth,
...
def sampleCandleStick():
    ...
    I = range(len(boxMid))
    chart.data = [boxMid]
    lines[0].symbol = candles = CandleSticks(chart=chart, boxFillColor=boxFillColor, boxWidth=boxWidth,
                                             crossWidth=crossWidth, strokeWidth=candleStrokeWidth,
                                             strokeColor=candleStrokeColor)
    for i in I: candles[i].setProperties(
        dict(position=i, boxMid=boxMid[i], crossLo=lo[i], crossHi=hi[i], boxLo=boxLo[i], boxHi=boxHi[i]))
    return d
