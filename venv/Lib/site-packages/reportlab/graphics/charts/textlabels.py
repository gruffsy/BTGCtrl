...
def _text2Path(text, x=0, y=0, fontName=_baseGFontName, fontSize=1000,
    ...
    return definePath(_text2PathDescription(text, x=x, y=y, fontName=fontName,
                                            fontSize=fontSize, anchor=anchor, truncate=truncate,
                                            pathReverse=pathReverse), **kwds)


_BA2TA = {'w': 'start', 'nw': 'start', 'sw': 'start', 'e': 'end', 'ne': 'end', 'se': 'end', 'n': 'middle',
          's': 'middle', 'c': 'middle'}
...
class PMVLabel(Label):
    ...
    def _getBoxAnchor(self):
        ...
        if self._pmv < 0: a = \
        {'nw': 'se', 'n': 's', 'ne': 'sw', 'w': 'e', 'c': 'c', 'e': 'w', 'sw': 'ne', 's': 'n', 'se': 'nw'}[a]
        return a
...
class BarChartLabel(PMVLabel):
    ...
    _attrMap = AttrMap(
        ...
        boxTarget=AttrMapValue(OneOf('normal', 'anti', 'lo', 'hi', 'mid'),
