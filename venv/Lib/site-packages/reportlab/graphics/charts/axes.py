...
from reportlab.graphics.shapes import Drawing, Line, PolyLine, Rect, Group, STATE_DEFAULTS, _textBoxLimits, \
    _rotatedBoxLimits
...
class _AxisG(Widget):
    ...
    def _makeLines(self, g, start, end, strokeColor, strokeWidth, strokeDashArray, strokeLineJoin, strokeLineCap,
                   strokeMiterLimit, parent=None, exclude=[], specials={}):
        ...
        if w and c and self.visibleGrid:
            ...
            if s or e:
                if self.isYAxis:
                    ...
                    offs = self._y
                self._makeLines(g, s - offs, e - offs, c, w, self.gridStrokeDashArray, self.gridStrokeLineJoin,
                                self.gridStrokeLineCap, self.gridStrokeMiterLimit, parent=parent, exclude=exclude,
    ...
    def _makeSubGrid(self, g, dim=None, parent=None, exclude=[]):
        ...
        if s or e:
            ...
            try:
                self._makeLines(g, s - offs, e - offs, c, w, self.subGridStrokeDashArray, self.subGridStrokeLineJoin,
                                self.subGridStrokeLineCap, self.subGridStrokeMiterLimit, parent=parent, exclude=exclude)
            finally:
...
class CALabel(PMVLabel):
    _attrMap = AttrMap(BASE=PMVLabel,
                       labelPosFrac=AttrMapValue(isNumber,
...
class CategoryAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        labelAxisMode=AttrMapValue(OneOf('high', 'low', 'axis', 'axispmv'),
        ...
        if itd:
            ...
            if tW:
                if tU and not tD:
                    ...
                    tU = tW * sW
            self._makeLines(g, tU, -tD, self.actualTickStrokeColor, sW, self.strokeDashArray, self.strokeLineJoin,
                            self.strokeLineCap, self.strokeMiterLimit)
...
class ValueAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        avoidBoundFrac=AttrMapValue(EitherOr((isNumberOrNone, SequenceOf(isNumber, emptyOK=0, lo=2, hi=2))),
                                    desc='Fraction of interval to allow above and below.'),
        avoidBoundSpace=AttrMapValue(EitherOr((isNumberOrNone, SequenceOf(isNumber, emptyOK=0, lo=2, hi=2))),
                                     desc='Space to allow above and below.'),
        abf_ignore_zero=AttrMapValue(EitherOr((NoneOr(isBoolean), SequenceOf(isBoolean, emptyOK=0, lo=2, hi=2))),
        ...
        tickStrokeColor=AttrMapValue(isColorOrNone, desc='Color of ticks if specified.'),
        keepTickLabelsInside=AttrMapValue(isBoolean,
...
class NormalDateXValueAxis(XValueAxis):
    ...
    def _xAxisTicker(self, xVals):
        ...
        if self.specifiedTickDates:
            ...
            if self.forceFirstDate and firstDate == ticks[0] and (axisLength / float(ticks[-1] - ticks[0])) * (
                    ticks[1] - ticks[0]) <= W:
                if self.specialTickClear:
                    labels[1] = ''
                else:
                    del ticks[1], labels[1]
            if self.forceEndDate and endDate == ticks[-1] and (axisLength / float(ticks[-1] - ticks[0])) * (
                    ticks[-1] - ticks[-2]) <= W:
                if self.specialTickClear:
                    labels[-2] = ''
                else:
...
class AdjYValueAxis(YValueAxis):
    ...
    _attrMap = AttrMap(BASE=YValueAxis,
                       ...
                       leftAxisSkipLL0=AttrMapValue(EitherOr((isBoolean, isListOfNumbers)),
...
class LogValueAxis(ValueAxis):
    ...
    def _setRange(self, dataSeries):
        ...
        if valueMin > valueMax:
            raise ValueError(
                '%s: valueMin=%r should not be greater than valueMax=%r!' % (self.__class__.__name__valueMin, valueMax))
        if valueMin <= 0:
