...
"""
    ...
    def draw(self):
        ...
        tickLine = shapes.PolyLine(
            points=[self.x + (s * 0.15), self.y + (s * 0.35), self.x + (s * 0.35), self.y + (s * 0.15),
...
class NoEntry(_Symbol):
    ...
    def draw(self):
        ...
        if self.strokeColor:
            g.add(shapes.Circle(cx=(self.x + (s / 2)), cy=(self.y + (s / 2)), r=s / 2, fillColor=None,
                                strokeColor=self.strokeColor, strokeWidth=1))

        if self.fillColor:
            g.add(shapes.Circle(cx=(self.x + (s / 2)), cy=(self.y + (s / 2)), r=((s / 2) - (s / 50)),
                                fillColor=self.fillColor, strokeColor=None, strokeWidth=0))

        innerBarColor = self.innerBarColor
        if innerBarColor:
            g.add(shapes.Rect(self.x + (s * 0.1), self.y + (s * 0.4), width=s * 0.8, height=s * 0.2,
                              fillColor=innerBarColor, strokeColor=innerBarColor, strokeLineCap=1, strokeWidth=0))
        return g
...
class NotAllowed(_Symbol):
    ...
    def draw(self):
        ...
        outerCircle = shapes.Circle(cx=(self.x + (s / 2)), cy=(self.y + (s / 2)), r=(s / 2) - (s / 10),
                                    fillColor=self.fillColor, strokeColor=strokeColor, strokeWidth=s / 10.)
        g.add(outerCircle)
...
class FloppyDisk(_Symbol):
    ...
    def draw(self):
        ...
        line1 = shapes.Line(x1=self.x + (s * 0.15), y1=self.y + (0.6 * s), x2=self.x + (s * 0.85),
                            y2=self.y + (0.6 * s),
        ...
        line2 = shapes.Line(x1=self.x + (s * 0.15), y1=self.y + (0.7 * s), x2=self.x + (s * 0.85),
                            y2=self.y + (0.7 * s),
                            fillColor=colors.black,
                            strokeColor=colors.black,
                            strokeWidth=0)
        ...
        line3 = shapes.Line(x1=self.x + (s * 0.15), y1=self.y + (0.8 * s), x2=self.x + (s * 0.85),
                            y2=self.y + (0.8 * s),
                            fillColor=colors.black,
                            strokeColor=colors.black,
                            strokeWidth=0)
