...
class Canvas(_PDFColorSetter):
    ...
    def setEncrypt(self, encrypt):
        ...
        if encrypt:
            ...
            if isStr(encrypt):  # encrypt is the password itself
                ...
                raise TypeError(
                    'Expected string or instance of reportlab.lib.pdfencrypt.StandardEncryption as encrypt parameter but got %r' % encrypt)
    ...
    def drawImage(self, image, x, y, width=None, height=None, mask=None,
        ...
        x, y, width, height, scaled = aspectRatioFix(preserveAspectRatio, anchor, x, y, width, height, imgObj.width,
                                                     imgObj.height, anchorAtXY)
    ...
    def _restartAccumulators(self):
        if self._codeStack:
            ...
            self._code, self._formsinuse, self._annotationrefs, self._formData, self._colorsUsed, self._shadingUsed = self._codeStack.pop(
                -1)
        else:
    ...
    def _pushAccumulators(self):
        ...
        saved = (
        self._code, self._formsinuse, self._annotationrefs, self._formData, self._colorsUsed, self._shadingUsed)
    ...
    def transform(self, a, b, c, d, e, f):
        ...
        if self._code and self._code[-1][-3:] == ' cm':
            ...
            self._code[-1] = s % fp_str(a0 * a + c0 * b, b0 * a + d0 * b, a0 * c + c0 * d, b0 * c + d0 * d,
                                        a0 * e + c0 * f + e0, b0 * e + d0 * f + f0)
        else:
    ...
    def cross(self, x, y, size=5, gap=1, text=None, strokeColor=None, strokeWidth=None, fontSize=3):
        size *= 0.5
        gap *= 0.5
        crosshairs = [(x, y - size, x, y - gap), (x, y + gap, x, y + size), (x - size, y, x - gap, y),
                      (x + gap, y, x + size, y)]
        self.saveState()
        if strokeColor:
